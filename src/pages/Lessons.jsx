...
export default function Lessons() {
  ...
  useEffect(() => {
    const load = async () => {
      const [{ data: studentsData, error: studentsError }, { data: lessonsData, error: lessonsError }] =
        await Promise.all([
      ...
      <main className="mx-auto max-w-3xl space-y-8 px-6 py-8">
        {students.length === 0 && !loading ? (
          <p className="rounded-md border border-gray-200 bg-white p-5 text-sm text-gray-500">
            ...
            <Link to="/students" className="font-medium text-indigo-600 hover:text-indigo-700">
              Add a student →
            </Link>
          </p>
        ) : (
          ...
          >
            <h2 className="text-base font-semibold text-gray-900">Log a lesson</h2>
        ...
        <section>
          <h2 className="mb-3 text-base font-semibold text-gray-900">Recent lessons</h2>
          {loading ? (
                ...
                <tbody className="divide-y divide-gray-200">
                  {lessons.map((l) => (
                    <tr key={l.id}>
                      ...
                      <td className="px-4 py-3 text-gray-700">{l.students?.name}</td>

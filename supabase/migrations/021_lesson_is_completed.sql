-- Bug: lessons were being treated as "completed" purely because their date
-- had arrived (autoCompletePastLessons() flipped status based on
-- lesson_date/lesson_time alone), which fed straight into payment-cycle
-- billing. A lesson should only count towards billing once the tutor has
-- explicitly tapped "Mark as Done". Introduce is_completed as the single
-- source of truth for that explicit confirmation, decoupled from the date.

alter table lessons
  add column if not exists is_completed boolean not null default false;

-- One-time backfill: lessons dated before today were genuinely completed
-- before this feature existed, so treat them as done. Lessons dated today
-- that a tutor had already explicitly marked 'completed' (via the existing
-- Mark as Done button) are preserved too. Everything else (today onwards,
-- still 'scheduled') stays false until the tutor marks it done.
update lessons
set is_completed = true
where lesson_date < current_date
   or (lesson_date = current_date and status = 'completed');
...
-- Re-run for every student so cycles realign with the new is_completed
-- backfill (mirrors the re-run pattern from migration 009).
do $$
declare
  r record;
begin

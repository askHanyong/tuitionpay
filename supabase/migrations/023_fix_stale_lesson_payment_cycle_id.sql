-- Bug: recompute_payment_cycles() only cleared payment_cycle_id from lessons
-- that ARE is_completed, before reassigning them to fresh cycles. It never
-- cleared payment_cycle_id from lessons that are NOT completed. So a lesson
-- that was once attached to a cycle (e.g. under the old status='completed'
-- logic, or after being un-marked) kept a stale payment_cycle_id forever,
-- and any query joining lessons -> payment_cycles by payment_cycle_id picked
-- it up regardless of is_completed -- which is exactly how a still-scheduled,
-- not-yet-completed lesson kept showing up as part of a "payment due" cycle.
--
-- Clear payment_cycle_id on every one of the student's lessons (not just the
-- completed ones) before reassigning, so an incomplete lesson can never
-- retain a link to a cycle.
...
-- Re-run for every student so stale payment_cycle_id links on incomplete
-- lessons (and any resulting orphaned cycles) are cleared immediately.
do $$
declare
  r record;
begin

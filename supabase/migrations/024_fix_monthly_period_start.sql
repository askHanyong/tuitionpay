-- Bug: for monthly/custom_date billing, recompute_payment_cycles() set a
-- cycle's period_start to the *first lesson's* date that month, not the
-- first calendar day of the month. So a student whose first lesson in May
-- fell on the 9th showed a payment period of "09 May to 31 May" instead of
-- the full calendar month "01 May to 31 May" that monthly billing actually
-- covers.
...
-- Re-run for every student so existing monthly/custom_date cycles get their
-- period_start corrected to the first of the month immediately.
do $$
declare
  r record;
begin
